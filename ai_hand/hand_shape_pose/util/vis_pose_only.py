...
def draw_3d_skeleton(pose_cam_xyz, image_size, elev_angle=-85, azim_angle=-75):
    ...
    for joint_ind in range(pose_cam_xyz.shape[0]):
        ...
        if joint_ind == 0:
            ...
            ax.plot(pose_cam_xyz[[0, joint_ind], 0], pose_cam_xyz[[0, joint_ind], 1], pose_cam_xyz[[0, joint_ind], 2], color=color_hand_joints[joint_ind], linewidth=line_wd)
        else:
...
def draw_batch_image_with_mesh_joints(batch_images, est_pose_uv,
                                      est_pose_cam_xyz,
                                      padding=2):
    """
    :param mesh_renderer:
