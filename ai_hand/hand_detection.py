...
while(True):
    ...
    est_pose_uv = est_pose_uv.to('cuda')
    est_pose_cam_xyz = est_pose_cam_xyz.to('cuda')
    ...
    avg_est_pose_uv += est_pose_uv[0].detach().cpu().numpy()
    avg_est_pose_cam_xyz += est_pose_cam_xyz[0].detach().cpu().numpy()
    avg_frame += 1
    ...
    if avg_frame == avg_per_frame:
        ...
        # plot hand poses
        cv2.imshow('hand pose estimation', skeleton_frame)#np.concatenate((skeleton_frame,skeleton_3D[:,:,:3]),axis=1))
        avg_est_pose_cam_xyz = np.zeros((21,3))
        avg_est_pose_uv = np.zeros((21,2))
